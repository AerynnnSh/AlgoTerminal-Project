...
import { useWatchlist } from "@/store/useWatchlist"; // Import Store
...
export default function LiveTable({ initialData }: LiveTableProps) {
  ...
  // Handler agar saat klik Bintang, tidak pindah halaman
  const handleStarClick = (e: React.MouseEvent, id: string) => {
    e.stopPropagation(); // Mencegah trigger ke onClick baris (router.push)
    toggleCoin(id);
  };
  ...
  return (
    <div className="relative border border-slate-800 rounded-lg overflow-hidden bg-slate-900/50">
      <div className="absolute top-0 right-0 p-2 z-10">
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${isUpdating ? "bg-emerald-400 animate-pulse" : "bg-slate-600"}`}
          ></span>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm mt-6 md:mt-0">
          <thead className="bg-slate-900 text-slate-400 font-mono uppercase text-xs border-b border-slate-800">
            <tr>
              <th className="p-4 w-10 text-center">★</th> {/* Kolom Bintang */}
          ...
          <tbody className="divide-y divide-slate-800">
            {coins.map((coin) => {
              // Cek apakah koin ini ada di watchlist
              ...
              return (
                ...
                >
                  ...
                  {/* 4. Change */}
                  <td
                    className={`p-4 text-right font-mono font-bold ${coin.price_change_percentage_24h >= 0 ? "text-emerald-400" : "text-rose-500"}`}
                  >
